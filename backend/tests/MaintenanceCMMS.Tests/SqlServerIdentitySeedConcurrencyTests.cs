...
public sealed class SqlServerIdentitySeedConcurrencyTests
{
    [Fact]
    public async Task SeedAsync_IsIdempotent_RepairsDifferentialChanges_AndSerializesConcurrentSeeders()
    {
        ...
        await using (var context = database.NewContext())
        {
            ...
            var roles = await new SqlServerIdentityStore(context).ListRolesAsync(CancellationToken.None);
    ...
    private static ServiceProvider CreateServices(string databaseName, string adminConnectionString)
    {
        ...
        services.AddDbContext<CmmsDbContext>(options => options.UseSqlServer(
            SqlServerWorkTestFixture.ConnectionString(adminConnectionString, databaseName)));
