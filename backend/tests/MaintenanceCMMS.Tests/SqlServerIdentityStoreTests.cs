...
public sealed class SqlServerIdentityStoreTests
{
    [Fact]
    public async Task UpsertUserAsync_SynchronizesRolesAndFaenasById_AndReactivatesHistoricalAssignments()
    {
        ...
        await using (var context = fixture.NewContext())
        {
            ...
            var assignments = await context.UserRoles
                .Where(assignment => assignment.UserId == SqlServerWorkTestFixture.PlannerUserId)
                .ToListAsync();
        ...
        await using (var context = fixture.NewContext())
        {
            ...
            var plannerAssignment = await context.UserRoles.SingleAsync(assignment =>
                assignment.UserId == SqlServerWorkTestFixture.PlannerUserId && assignment.RoleId == plannerRole.Id);
        ...
        await using (var context = fixture.NewContext())
        {
            ...
            var supervisorAssignment = await context.UserRoles.SingleAsync(assignment =>
                assignment.UserId == SqlServerWorkTestFixture.PlannerUserId && assignment.RoleId == supervisorRole.Id);
        ...
        await using (var context = fixture.NewContext())
        {
            var assignment = await context.UserFaenas.SingleAsync(item => item.UserId == SqlServerWorkTestFixture.PlannerUserId);
        ...
        await using (var context = fixture.NewContext())
        {
            var assignments = await context.UserFaenas
                .Where(item => item.UserId == SqlServerWorkTestFixture.PlannerUserId)
                .ToListAsync();
    ...
    public async Task UpsertUserAsync_RejectsUnknownRoleAndFaenaWithDomainExceptions()
    {
        ...
        await using (var context = fixture.NewContext())
        {
            var store = new SqlServerIdentityStore(context);
    ...
    private static async Task<UserAccount> GetPlannerAsync(SqlServerWorkTestFixture fixture)
    {
        await using var context = fixture.NewContext();
        var user = await new SqlServerIdentityStore(context)
            .FindUserByIdAsync(SqlServerWorkTestFixture.PlannerUserId.ToString("D"), CancellationToken.None);

        return Assert.IsType<UserAccount>(user);
    }

    private static async Task UpsertAsync(SqlServerWorkTestFixture fixture, UserAccount user)
    {
        await using var context = fixture.NewContext();
        await new SqlServerIdentityStore(context).UpsertUserAsync(user, CancellationToken.None);
    }
}
