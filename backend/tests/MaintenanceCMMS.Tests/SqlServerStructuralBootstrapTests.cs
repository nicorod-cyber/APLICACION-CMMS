...
public sealed class SqlServerStructuralBootstrapTests
{
    [Fact]
    public async Task BootstrapAsync_CreatesOnlyStructuralRecords_AndIsIdempotent()
    {
        ...
        await using var db = new CmmsDbContext(new DbContextOptionsBuilder<CmmsDbContext>()
            .UseSqlServer(
                SqlServerWorkTestFixture.ConnectionString(adminConnectionString, name),
        ...
        try
        {
            ...
            var bootstrap = new SqlServerStructuralBootstrap(
                db,
                new PasswordHasher(),
            ...
            Assert.Equal(0, first.Roles);
            Assert.Equal(0, first.Permissions);
            Assert.Equal(0, first.RolePermissions);
        ...
        finally
        {
            ...
            await SqlServerWorkTestFixture.DropDatabaseAsync(name, adminConnectionString);
        }
    }
