...
public sealed class TechnicalHierarchyServiceTests
{
    ...
    public async Task Importer_UpdatesTheUniqueLocationBeforeNodesAndIsIdempotent()
    {
        ...
        var importer=new TechnicalHierarchyExcelImportService(fx.Db,new AuthorizationPolicyService(),new SqlServerAuditService(fx.Db,new AuditContextAccessor()));
    ...
    {
        private Fixture(string name,string adminConnectionString,CmmsDbContext db){Name=name;AdminConnectionString=adminConnectionString;Db=db;Service=new TechnicalHierarchyService(db,new SqlServerAuditService(db,new AuditContextAccessor()),new AuthorizationPolicyService());}
        ...
        {
            var name=$"cmms_test_th_{Guid.NewGuid():N}";var adminConnectionString=await SqlServerWorkTestFixture.GetAdminConnectionStringAsync();await SqlServerWorkTestFixture.CreateDatabaseAsync(name,adminConnectionString);
        ...
        private static CmmsDbContext CreateContext(string name,string adminConnectionString)=>new(new DbContextOptionsBuilder<CmmsDbContext>().UseSqlServer(SqlServerWorkTestFixture.ConnectionString(adminConnectionString,name)).Options);
        ...
        public async ValueTask DisposeAsync(){await Db.DisposeAsync();await SqlServerWorkTestFixture.DropDatabaseAsync(Name,AdminConnectionString);}
    }
}
