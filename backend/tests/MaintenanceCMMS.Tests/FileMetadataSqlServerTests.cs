...
public sealed class FileMetadataSqlServerTests
{
    [Fact]
    public async Task SaveGetListAndDownload_UseSqlServerAcrossContexts()
    {
    ...
    private static LocalSharePointSimulationService CreateStorage(
        MaintenanceCMMS.Infrastructure.Data.SqlServer.CmmsDbContext dbContext,
        string root)
    {
        return new LocalSharePointSimulationService(
            dbContext,
            new SqlServerAuditService(dbContext, new AuditContextAccessor()),
