...
public sealed class AssetReadingConfiguration : IEntityTypeConfiguration<AssetReadingEntity>
{
    ...
    {
        b.ToTable("lecturas_activo"); b.ConfigureBase(); b.Property(x=>x.AssetId).HasColumnName("activo_id"); b.Property(x=>x.ReadAtUtc).HasColumnName("fecha_lectura_utc").HasColumnType("datetimeoffset"); b.Property(x=>x.Value).HasColumnName("valor").HasPrecision(18,2); b.Property(x=>x.Source).HasColumnName("origen").HasMaxLength(40); b.Property(x=>x.WorkOrderId).HasColumnName("orden_trabajo_id"); b.Property(x=>x.OperationalUnitId).HasColumnName("unidad_operativa_id"); b.Property(x=>x.OperationalUnitReadingOperationId).HasColumnName("operacion_lectura_unidad_id"); b.Property(x=>x.IsCompositionSynchronization).HasColumnName("es_sincronizacion_composicion"); b.Property(x=>x.RegisteredByUserId).HasColumnName("registrado_por_usuario_id").HasMaxLength(120); b.Property(x=>x.EvidenceReference).HasColumnName("evidencia_referencia").HasMaxLength(1000); b.Property(x=>x.Observations).HasColumnName("observaciones").HasMaxLength(1000); b.Property(x=>x.IsCorrection).HasColumnName("es_correccion"); b.Property(x=>x.CorrectedReadingId).HasColumnName("lectura_corregida_id"); b.Property(x=>x.CorrectionReason).HasColumnName("motivo_correccion").HasMaxLength(1000); b.Property(x=>x.AuthorizedByUserId).HasColumnName("autorizado_por_usuario_id").HasMaxLength(120); b.Property(x=>x.IsAnomalous).HasColumnName("es_anomala"); b.Property(x=>x.ValidationMessage).HasColumnName("mensaje_validacion").HasMaxLength(1000); b.HasIndex(x=>new{x.AssetId,x.ReadAtUtc}); b.HasIndex(x=>x.OperationalUnitReadingOperationId); b.HasOne(x=>x.Asset).WithMany().HasForeignKey(x=>x.AssetId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.WorkOrder).WithMany().HasForeignKey(x=>x.WorkOrderId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.OperationalUnit).WithMany().HasForeignKey(x=>x.OperationalUnitId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.CorrectedReading).WithMany().HasForeignKey(x=>x.CorrectedReadingId).OnDelete(DeleteBehavior.Restrict); b.ToTable(t => { t.HasCheckConstraint("ck_lecturas_activo_valor", "valor >= 0"); t.HasCheckConstraint("ck_lecturas_activo_origen", "origen IN ('MANUAL','ORDEN_TRABAJO','IMPORTACION','SAP','TELEMETRIA')"); });
    }
}
...
{
    public void Configure(EntityTypeBuilder<OperationalUnitEntity> b)
    {
        b.ToTable("unidades_operativas"); b.ConfigureBase();
        ...
        b.Property(x=>x.BaselineOperationalStateId).HasColumnName("estado_operacional_base_id"); b.Property(x=>x.DerivedFromAssetId).HasColumnName("estado_derivado_por_activo_id"); b.Property(x=>x.DerivedStateReason).HasColumnName("motivo_estado_derivado").HasMaxLength(1000); b.Property(x=>x.DerivedStateCalculatedAtUtc).HasColumnName("estado_derivado_calculado_utc").HasColumnType("datetimeoffset");
        b.Property(x=>x.Criticality).HasColumnName("criticidad").HasMaxLength(40); b.Property(x=>x.CommissioningDate).HasColumnName("fecha_puesta_servicio"); b.Property(x=>x.DecommissioningDate).HasColumnName("fecha_baja"); b.Property(x=>x.Observations).HasColumnName("observaciones").HasMaxLength(2000);
        b.HasIndex(x=>x.Code).IsUnique(); b.HasOne(x=>x.OperationalUnitType).WithMany().HasForeignKey(x=>x.OperationalUnitTypeId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.Faena).WithMany().HasForeignKey(x=>x.FaenaId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.OperationalState).WithMany().HasForeignKey(x=>x.OperationalStateId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.BaselineOperationalState).WithMany().HasForeignKey(x=>x.BaselineOperationalStateId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.DerivedFromAsset).WithMany().HasForeignKey(x=>x.DerivedFromAssetId).OnDelete(DeleteBehavior.Restrict);
        b.ToTable(t=>t.HasCheckConstraint("ck_unidades_operativas_fecha_baja", "fecha_baja IS NULL OR fecha_puesta_servicio IS NULL OR fecha_baja >= fecha_puesta_servicio"));
    }
}
...
{
    public void Configure(EntityTypeBuilder<OperationalUnitComponentRoleEntity> b) { b.ToTable("roles_componente_unidad"); b.ConfigureBase(); b.Property(x=>x.Code).HasColumnName("codigo").HasMaxLength(60).IsRequired(); b.Property(x=>x.Name).HasColumnName("nombre").HasMaxLength(160).IsRequired(); b.Property(x=>x.Description).HasColumnName("descripcion").HasMaxLength(500); b.Property(x=>x.IsCritical).HasColumnName("critico"); b.Property(x=>x.IsActive).HasColumnName("activo"); b.HasIndex(x=>x.Code).IsUnique(); }
}
...
{
    public void Configure(EntityTypeBuilder<OperationalUnitComponentEntity> b)
    {
        b.ToTable("componentes_unidad_operativa"); b.ConfigureBase(); b.Property(x=>x.OperationalUnitId).HasColumnName("unidad_operativa_id"); b.Property(x=>x.AssetId).HasColumnName("activo_id"); b.Property(x=>x.ComponentRoleId).HasColumnName("rol_componente_id"); b.Property(x=>x.InstalledAtUtc).HasColumnName("fecha_montaje_utc").HasColumnType("datetimeoffset"); b.Property(x=>x.RemovedAtUtc).HasColumnName("fecha_desmontaje_utc").HasColumnType("datetimeoffset"); b.Property(x=>x.InstallationWorkOrderId).HasColumnName("orden_trabajo_montaje_id"); b.Property(x=>x.RemovalWorkOrderId).HasColumnName("orden_trabajo_desmontaje_id");
        b.Property(x=>x.InstalledByUserId).HasColumnName("montado_por_usuario_id").HasMaxLength(120).IsRequired(); b.Property(x=>x.InstallationReason).HasColumnName("motivo_montaje").HasMaxLength(500); b.Property(x=>x.RemovedByUserId).HasColumnName("desmontado_por_usuario_id").HasMaxLength(120); b.Property(x=>x.RemovalReason).HasColumnName("motivo_desmontaje").HasMaxLength(500); b.Property(x=>x.CriticalRoleCode).HasColumnName("rol_critico_codigo").HasMaxLength(60); b.Property(x=>x.Observations).HasColumnName("observaciones").HasMaxLength(1000);
        b.HasIndex(x=>x.AssetId).IsUnique().HasFilter("fecha_desmontaje_utc IS NULL"); b.HasIndex(x=>new{x.OperationalUnitId,x.CriticalRoleCode}).IsUnique().HasFilter("fecha_desmontaje_utc IS NULL AND rol_critico_codigo IS NOT NULL");
        b.HasOne(x=>x.OperationalUnit).WithMany().HasForeignKey(x=>x.OperationalUnitId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.Asset).WithMany().HasForeignKey(x=>x.AssetId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.ComponentRole).WithMany().HasForeignKey(x=>x.ComponentRoleId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.InstallationWorkOrder).WithMany().HasForeignKey(x=>x.InstallationWorkOrderId).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.RemovalWorkOrder).WithMany().HasForeignKey(x=>x.RemovalWorkOrderId).OnDelete(DeleteBehavior.Restrict);
        b.ToTable(t=> { t.HasCheckConstraint("ck_componentes_unidad_fechas", "fecha_desmontaje_utc IS NULL OR fecha_desmontaje_utc >= fecha_montaje_utc"); t.HasCheckConstraint("ck_componentes_unidad_rol_critico", "rol_critico_codigo IS NULL OR rol_critico_codigo IN ('FABRICA','CHASIS')"); });
    }
}
