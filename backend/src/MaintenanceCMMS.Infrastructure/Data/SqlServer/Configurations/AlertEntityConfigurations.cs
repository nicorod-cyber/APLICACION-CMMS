...
public sealed class AlertConfiguration : IEntityTypeConfiguration<AlertEntity>
{
    public void Configure(EntityTypeBuilder<AlertEntity> builder)
    {
        ...
        builder.Property(x => x.FaenaId).HasColumnName("faena_id"); builder.Property(x => x.EntityType).HasColumnName("tipo_entidad").HasMaxLength(120); builder.Property(x => x.EntityId).HasColumnName("entidad_id").HasMaxLength(240); builder.Property(x => x.IsCriticalRepeat).HasColumnName("repeticion_critica"); builder.Property(x => x.RepeatCount).HasColumnName("cantidad_repeticiones"); builder.Property(x => x.AcknowledgedAtUtc).HasColumnName("reconocido_at_utc").HasColumnType("datetimeoffset"); builder.Property(x => x.AcknowledgedByUserId).HasColumnName("reconocido_por_usuario_id").HasMaxLength(120); builder.Property(x => x.ResolvedAtUtc).HasColumnName("resuelto_at_utc").HasColumnType("datetimeoffset"); builder.Property(x => x.ResolvedByUserId).HasColumnName("resuelto_por_usuario_id").HasMaxLength(120); builder.Property(x => x.ResolutionReason).HasColumnName("motivo_resolucion").HasMaxLength(1000); builder.Property(x => x.IsActive).HasColumnName("activa"); builder.Property(x => x.GeneratedPdfFileId).HasColumnName("archivo_pdf_id");
        builder.HasIndex(x => new { x.AlertRuleId, x.DeduplicationKey, x.IsActive }).IsUnique().HasFilter("activa = 1"); builder.HasIndex(x => new { x.EntityType, x.EntityId }); builder.HasIndex(x => x.FaenaId); builder.HasIndex(x => new { x.Status, x.Severity });
...
public sealed class NotificationAttemptConfiguration : IEntityTypeConfiguration<NotificationAttemptEntity>
{
    public void Configure(EntityTypeBuilder<NotificationAttemptEntity> builder)
    {
        ...
        builder.Property(x => x.NotificationId).HasColumnName("notificacion_id"); builder.Property(x => x.AttemptNumber).HasColumnName("numero_intento"); builder.Property(x => x.AttemptedAtUtc).HasColumnName("intentado_at_utc").HasColumnType("datetimeoffset"); builder.Property(x => x.Success).HasColumnName("exitoso"); builder.Property(x => x.Provider).HasColumnName("proveedor").HasMaxLength(120); builder.Property(x => x.Error).HasColumnName("error").HasMaxLength(2000);
