...
/// </summary>
public interface ISqlServerImportHandler
{
    string SchemaName { get; }

    Task<IReadOnlyCollection<SqlServerImportRowResult>> AnalyzeAsync(
        IReadOnlyCollection<SqlServerImportRow> rows,
...
public sealed record SqlServerImportRow(int RowNumber, IReadOnlyDictionary<string, string?> Values);

public sealed record SqlServerImportRowResult(
    int RowNumber,
    string Operation,
    IReadOnlyCollection<ExcelImportValidationError> Errors);

public sealed class SqlServerImportHandlerResolver(IEnumerable<ISqlServerImportHandler> handlers)
{
    private readonly IReadOnlyDictionary<string, ISqlServerImportHandler> _handlers = handlers
        .ToDictionary(handler => handler.SchemaName, StringComparer.OrdinalIgnoreCase);

    public ISqlServerImportHandler GetRequired(string schemaName) =>
        _handlers.TryGetValue(schemaName, out var handler)
            ? handler
            : throw new DomainException($"El esquema '{schemaName}' no tiene un importador SQL Server habilitado.");
}

public abstract class SqlServerImportHandlerBase(CmmsDbContext db) : ISqlServerImportHandler
{
    protected CmmsDbContext Db { get; } = db;
    public abstract string SchemaName { get; }

    public abstract Task<IReadOnlyCollection<SqlServerImportRowResult>> AnalyzeAsync(
        IReadOnlyCollection<SqlServerImportRow> rows,
        CancellationToken cancellationToken);
    ...
    protected static ExcelImportValidationError Error(SqlServerImportRow row, string column, string message) =>
        new(row.RowNumber, column, message);

    protected static IReadOnlyCollection<SqlServerImportRowResult> DuplicateKeyErrors(
        IReadOnlyCollection<SqlServerImportRow> rows,
        string keyColumn)
    {
        var duplicates = rows
        ...
        return rows.Select(row => new SqlServerImportRowResult(
            row.RowNumber,
...
public sealed class FaenaSqlServerImportHandler(CmmsDbContext db) : SqlServerImportHandlerBase(db)
{
    public override string SchemaName => "faenas";

    public override async Task<IReadOnlyCollection<SqlServerImportRowResult>> AnalyzeAsync(IReadOnlyCollection<SqlServerImportRow> rows, CancellationToken ct)
    {
            ...
            var operation = errors.Count > 0
                ...
                : faena is null
                    ...
                    : IsSame(faena, currentLocation, row, latitude, longitude, responsible!)
                        ? "SinCambios"
                        : "Actualizado";
            return new SqlServerImportRowResult(row.RowNumber, operation, errors);
        }).ToArray();
    }

    public override async Task ApplyAsync(IReadOnlyCollection<SqlServerImportRow> rows, string appliedBy, CancellationToken ct)
    {
    ...
    private static void ValidateRequired(SqlServerImportRow row, ICollection<ExcelImportValidationError> errors, params string[] columns)
    {
    ...
    private static bool IsSame(FaenaEntity faena, TechnicalLocationEntity? location, SqlServerImportRow row, decimal? latitude, decimal? longitude, AppUserEntity responsible)
    {
...
public sealed class TechnicalLocationSqlServerImportHandler(CmmsDbContext db) : SqlServerImportHandlerBase(db)
{
...
public sealed class WarehouseSqlServerImportHandler(CmmsDbContext db) : SqlServerImportHandlerBase(db)
{
    ...
    public override async Task<IReadOnlyCollection<SqlServerImportRowResult>> AnalyzeAsync(IReadOnlyCollection<SqlServerImportRow> rows, CancellationToken ct)
    {
        ...
        return rows.Select(row =>
        {
            ...
            return new SqlServerImportRowResult(row.RowNumber, errors.Count > 0 ? "Error" : existing.Contains(code) ? "Actualizado" : "Nuevo", errors);
        }).ToArray();
    }
...
public sealed class SparePartSqlServerImportHandler(CmmsDbContext db) : SqlServerImportHandlerBase(db)
{
...
public sealed class AssetSqlServerImportHandler(CmmsDbContext db) : SqlServerImportHandlerBase(db)
{
    ...
    public override async Task<IReadOnlyCollection<SqlServerImportRowResult>> AnalyzeAsync(IReadOnlyCollection<SqlServerImportRow> rows, CancellationToken ct)
    {
        ...
        return rows.Select(row =>
        {
            ...
            return new SqlServerImportRowResult(row.RowNumber, errors.Count > 0 ? "Error" : existingCodes.Contains(code) ? "Actualizado" : "Nuevo", errors);
        }).ToArray();
    }
