...
public sealed class ExcelImportWorkflowService : IExcelImportWorkflowService
{
    ...
    public async Task<ExcelImportPreviewResult> UploadAsync(ExcelImportUploadCommand command, CancellationToken ct)
    {
        ...
        var entity = new ImportEntity { Id = id, EntityName = command.Entity.Trim(), SchemaName = schema.SchemaName, OriginalFileName = fileName, FileId = fileId, SimulateOnly = command.SimulateOnly, Status = (int)status, UploadedByUserId = command.UploadedBy.Trim(), UploadedAtUtc = DateTimeOffset.UtcNow,
            ...
            Events = [new ImportEventEntity { Status = (int)status, UserId = command.UploadedBy.Trim(), OccurredAtUtc = DateTimeOffset.UtcNow, Detail = $"Archivo recibido para {schema.SchemaName}. SimulaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n: {command.SimulateOnly}." }] };
    ...
    public async Task<ExcelImportPreviewResult?> ApproveAsync(string id, string approvedBy, CancellationToken ct)
    {
        ...
        if ((ImportStatus)entity.Status is ImportStatus.Applied or ImportStatus.Rejected) throw new DomainException("La importaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n ya fue cerrada.");
        ...
        if (entity.Errors.Count > 0) { entity.Status = (int)ImportStatus.Validating; _db.ImportEvents.Add(Event(entity.Id, entity.Status, approvedBy, "AprobaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n bloqueada por errores de validaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n actuales.")); await _db.SaveChangesAsync(ct); throw new DomainException("No se puede aprobar una importaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n con errores. Revise el preview actualizado."); }
        try
        {
            await using var transaction = await _db.Database.BeginTransactionAsync(ct);
            await _handlers.GetRequired(entity.SchemaName).ApplyAsync(rows, approvedBy.Trim(), ct);
            entity.Status = (int)ImportStatus.Applied; entity.AppliedAtUtc = DateTimeOffset.UtcNow; entity.AppliedByUserId = approvedBy.Trim(); entity.RejectReason = null; _db.ImportEvents.Add(Event(entity.Id, entity.Status, approvedBy, "ImportaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n aplicada al maestro relacional."));
            await _db.SaveChangesAsync(ct); await transaction.CommitAsync(ct);
        }
        catch (Exception ex) when (ex is not DomainException)
        {
            _db.ChangeTracker.Clear(); var failed = await LoadAsync(key, true, ct) ?? throw new InvalidOperationException("No se encontrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ la importaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n tras revertir la transacciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n."); failed.Status = (int)ImportStatus.Failed; failed.RejectReason = ex.Message.Length > 2000 ? ex.Message[..2000] : ex.Message; _db.ImportEvents.Add(Event(failed.Id, failed.Status, approvedBy, "La aplicaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n fallÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³; no se confirmaron cambios del maestro.")); await _db.SaveChangesAsync(ct); await AuditAsync(approvedBy, "import.failed", failed, AuditSeverity.Critical, ex.Message, ct); throw;
        }
        await AuditAsync(approvedBy, "import.applied", entity, AuditSeverity.Critical, "AprobaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n de importaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n Excel", ct); return ToPreview(entity);
    }
    ...
    public async Task<ExcelImportPreviewResult?> RejectAsync(string id, string rejectedBy, string? reason, CancellationToken ct)
    {
        ...
        await _db.Entry(entity).ReloadAsync(ct);
        if ((ImportStatus)entity.Status == ImportStatus.Applied) throw new DomainException("No se puede rechazar una importaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n ya aplicada.");
        entity.Status = (int)ImportStatus.Rejected; entity.RejectedAtUtc = DateTimeOffset.UtcNow; entity.RejectedByUserId = rejectedBy.Trim(); entity.RejectReason = Null(reason); _db.ImportEvents.Add(Event(entity.Id, entity.Status, rejectedBy, "ImportaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n rechazada.")); await _db.SaveChangesAsync(ct); await AuditAsync(rejectedBy, "import.rejected", entity, AuditSeverity.High, reason, ct); return ToPreview(entity);
    }
