...
public async Task<WorkOrderDetailResponse> CreatePreventiveAsync(CreatePreventiveWorkOrderRequest r, UserAccessContext u, CancellationToken ct)
    ...
    public async Task<WorkOrderEvidenceResponse?> RegisterEvidenceAsync(string numeroOt, RegisterEvidenceRequest r, UserAccessContext u, CancellationToken ct)
    {
        ...
        var e = new WorkOrderEvidenceEntity { Id = Guid.NewGuid(), WorkOrderId = o.Id, TaskId = task.Id, Name = r.Nombre.Trim(), EvidenceTypeId = (await CatalogAsync("WorkOrderEvidenceType", r.TipoEvidencia.ToString(), ct)).Id, IsPhoto = r.EsFoto || r.TipoEvidencia is WorkOrderEvidenceType.FotoAntes or WorkOrderEvidenceType.FotoDespues, IsMandatory = r.EsObligatoria, CoversMandatoryEvidence = r.CubreEvidenciaObligatoria, StorageProvider = N(r.StorageProvider) ?? Infer(r), ExternalKey = N(r.ArchivoKey), ExternalUri = string.IsNullOrWhiteSpace(r.SharePointUrl) ? null : DocumentUrlPolicy.RequireDocumentLink(r.SharePointUrl), LocalPath = N(r.LocalPath), OfflineId = N(r.OfflineId), SyncStatus = N(r.SyncStatus) ?? "Synced", Observations = N(r.Observaciones), UploadedByUserId = uploaderId, UploadedAtUtc = DateTimeOffset.UtcNow };
