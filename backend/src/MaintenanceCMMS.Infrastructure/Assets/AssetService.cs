...
public sealed class AssetService : IAssetService
{
    ...
    public async Task<AssetDetail?> UpdateAsync(string codigo, UpdateAssetRequest r, UserAccessContext u, CancellationToken ct)
    {
        ...
        var refs = await ReferencesAsync(r.TipoActivoCodigo, r.FamiliaEquipoCodigo, r.FaenaCodigo, r.UbicacionTecnicaCodigo, r.EstadoOperacionalCodigo, u, ct); var measurement = Measurement(r.TipoMedicionUso);
    ...
    private IQueryable<AssetEntity> Query() => _db.Assets.Include(x => x.AssetTypeDefinition).Include(x => x.Family).Include(x => x.Faena).Include(x => x.TechnicalLocation).Include(x => x.OperationalState);
    ...
    private async Task<(AssetTypeEntity Type, EquipmentFamilyEntity? Family, FaenaEntity? Faena, TechnicalLocationEntity? Location, AssetOperationalStateEntity State)> ReferencesAsync(string type, string? family, string? faena, string? location, string state, UserAccessContext u, CancellationToken ct)
    {
        ...
        var faenaEntity = faena is null or "" ? null : await _db.Faenas.SingleOrDefaultAsync(x => x.Code == Code(faena) && x.IsActive, ct) ?? throw new DomainException("Faena inexistente."); if (faenaEntity is not null && !_authorization.CanViewFaena(u, faenaEntity.Code)) throw new UnauthorizedAccessException("No tiene acceso a la faena indicada.");
        var locationEntity = location is null or "" ? null : await _db.TechnicalLocations.SingleOrDefaultAsync(x => x.Code == Code(location) && !x.IsObsolete, ct) ?? throw new DomainException("Ubicacion tecnica inexistente."); if (locationEntity is not null && (faenaEntity is null || locationEntity.FaenaId != faenaEntity.Id)) throw new DomainException("La ubicacion tecnica no corresponde a la faena.");
        var stateEntity = await _db.AssetOperationalStates.SingleOrDefaultAsync(x => x.Code == Code(state) && x.IsActive, ct) ?? throw new DomainException("Estado operacional inexistente."); return (typeEntity, familyEntity, faenaEntity, locationEntity, stateEntity);
    }
    ...
    private async Task AuditAsync(UserAccessContext u, string action, AssetEntity a, object? previous, object? next, CancellationToken ct) => await _audit.RecordAsync(new AuditEventRequest(u.UserId, action, AuditModules.Assets, "Asset", a.Code, previous is null ? null : JsonSerializer.Serialize(previous), next is null ? null : JsonSerializer.Serialize(next), a.Faena?.Code, AuditSeverity.Medium), ct);
