...
public sealed class SqlServerIdentitySeedTransaction : IIdentitySeedTransaction
{
    private const int MaxAttempts = 3;
    private const long RolePermissionSeedLockKey = 7_144_260_118_247_903_411;

    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<SqlServerIdentitySeedTransaction> _logger;

    public SqlServerIdentitySeedTransaction(
        IServiceScopeFactory scopeFactory,
        ILogger<SqlServerIdentitySeedTransaction> logger)
    {
        ...
        for (var attempt = 1; attempt <= MaxAttempts; attempt++)
        {
            ...
            try
            {
                await dbContext.Database.CreateExecutionStrategy().ExecuteAsync(async () =>
                {
                    ...
                    try
                    {
                        await SqlServerTransactionLock.AcquireExclusiveAsync(dbContext, "cmms.identity-seed", cancellationToken);
