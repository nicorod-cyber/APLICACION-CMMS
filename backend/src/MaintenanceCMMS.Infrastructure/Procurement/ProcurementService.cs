...
public sealed class ProcurementService : IProcurementService
{
    ...
    public async Task<ProcurementRequestResponse> CreateRequestAsync(CreateProcurementRequestRequest request, UserAccessContext user, CancellationToken ct)
    {
        ...
        entity.Lines.Add(new ProcurementRequestLineEntity
        {
            ...
            Unit = request.Unidad.Trim(), EstimatedCost = request.CostoEstimado, Currency = Text(request.Moneda) ?? "CLP", SupportingDocumentUrl = string.IsNullOrWhiteSpace(request.DocumentoRespaldoUrl) ? null : DocumentUrlPolicy.RequireDocumentLink(request.DocumentoRespaldoUrl), Notes = "Solicitud enviada a abastecimiento"
        });
    ...
    public async Task<ProcurementRequestResponse?> RegisterReceptionAsync(string id, RegisterProcurementReceptionRequest request, UserAccessContext user, CancellationToken ct)
    {
        ...
        var receipt = new ProcurementReceiptEntity { ProcurementRequestId = entity.Id, PurchaseOrderId = order.Id, WarehouseId = warehouse.Id, ReceivedAtUtc = request.FechaRecepcion ?? DateTimeOffset.UtcNow, DirectDispatchToWorkOrder = request.DespachoDirectoOt, ActualCost = request.CostoReal, ReceptionDocumentUrl = string.IsNullOrWhiteSpace(request.DocumentoRecepcionUrl) ? null : DocumentUrlPolicy.RequireDocumentLink(request.DocumentoRecepcionUrl), DeliveryDocumentUrl = string.IsNullOrWhiteSpace(request.DocumentoEntregaUrl) ? null : DocumentUrlPolicy.RequireDocumentLink(request.DocumentoEntregaUrl), CreatedByUserId = user.UserId, Reason = request.Reason.Trim(), ReceptionMovementId = await MovementIdAsync(receptionNumber, ct), DeliveryMovementId = await MovementIdAsync(deliveryNumber, ct) };
