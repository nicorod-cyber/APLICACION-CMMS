...
public sealed class AlertService : IAlertService
{
    ...
    {
        var template = rule.Template ?? await _templateService.ResolveActiveAsync("alert-default", cancellationToken); var data = BuildData(alert, rule.Code, extraData); var subject = PdfTemplateService.RenderSubject(template.SubjectTemplate, data); var body = PdfTemplateService.RenderTemplate(template.HtmlTemplate, data);
    ...
    private static NotificationResponse ToNotificationResponse(NotificationEntity item) => new(item.Id.ToString("D"), item.AlertId.ToString("D"), item.Subject, item.Body, string.Join(';', item.Recipients.Select(x => x.Destination).Where(x => !string.IsNullOrWhiteSpace(x))), Parse<NotificationStatus>(item.Status), item.CreatedAtUtc, item.SentAtUtc, item.Provider, item.PdfFile?.FileKey, item.PdfFile?.LogicalUri, item.LastError);
