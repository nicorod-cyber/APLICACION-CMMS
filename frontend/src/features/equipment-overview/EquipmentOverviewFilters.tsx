...
export function EquipmentOverviewFilters({ value, catalog, onChange, onApply, disabled }: {
  ...
  return <div className="border-b border-slate-200 p-4">
    <div className="grid gap-2 lg:grid-cols-[minmax(260px,1.3fr)_220px_180px_180px_auto]">
      <label className="relative"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><input className="input bg-slate-50 pl-9" placeholder="Buscar por nombre, cÃ³digo, patente o serie" value={value.search} onChange={event => onChange({ ...value, search: event.target.value })} onKeyDown={event => { if (event.key === "Enter") onApply(); }} /></label>
    ...
    {advanced ? <div className="mt-3 grid gap-2 border-t border-slate-100 pt-3 md:grid-cols-4"><input className="input" placeholder="Zona, ej. 3 o Zona 3" value={value.zona} onChange={event => onChange({ ...value, zona: event.target.value })} onKeyDown={event => { if (event.key === "Enter") onApply(); }} /><select className="input" value={value.tipoUbicacionFisica} onChange={event => onChange({ ...value, tipoUbicacionFisica: event.target.value })}><option value="">Toda ubicaciÃ³n</option><option value="FAENA">En faena</option><option value="TALLER">Taller</option></select><input className="input" placeholder="CÃ³digo de taller" value={value.tallerCodigo} onChange={event => onChange({ ...value, tallerCodigo: event.target.value })} /><button className="secondary-button" type="button" onClick={onApply} disabled={disabled}>Aplicar filtros</button></div> : <div className="mt-3"><button className="text-xs font-semibold text-teal-700 underline" type="button" disabled={disabled} onClick={onApply}>Aplicar filtros</button></div>}
  </div>;
}