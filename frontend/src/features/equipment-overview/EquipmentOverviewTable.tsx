import { Box, ChevronRight, Truck, Wrench } from "lucide-react";
...
function EquipmentIcon({ row }: { row: EquipmentOverviewRow }) {
  const shared = "h-5 w-5";
  const type = row.rowType === "COMPOSITE_UNIT" ? "bg-sky-100 text-sky-700" : row.rowType === "LOOSE_COMPONENT" ? "bg-orange-50 text-orange-700" : "bg-teal-50 text-teal-700";
  return <span className={"grid h-9 w-9 shrink-0 place-items-center rounded-[9px] " + type}>{row.rowType === "COMPOSITE_UNIT" ? <Truck className={shared} /> : row.rowType === "LOOSE_COMPONENT" ? <Wrench className={shared} /> : <Box className={shared} />}</span>;
}
  ...
  return <div className="max-h-[680px] overflow-auto">
    <table className="min-w-[2050px] text-left text-sm">
      ...
      <tbody>{rows.map(row => <tr className="cursor-pointer border-t border-slate-100 align-middle hover:bg-slate-50 focus-within:bg-teal-50" key={row.rowId} tabIndex={0} role="link" onClick={() => onOpen(row)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(row); } }}>
        ...
        <td className="sticky left-0 z-[1] min-w-[270px] bg-white p-3 dark:bg-slate-900"><div className="flex items-center gap-3"><EquipmentIcon row={row} /><div className="min-w-0"><b className="block truncate text-slate-900">{row.name || "NA"}</b><small className="block text-slate-500">{row.code}</small><span className={"mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-extrabold " + (row.rowType === "COMPOSITE_UNIT" ? "bg-sky-100 text-sky-800" : row.rowType === "LOOSE_COMPONENT" ? "bg-orange-50 text-orange-800" : "bg-teal-50 text-teal-800")}>{rowTypeLabel(row.rowType)}</span></div></div></td>
