...
type EquipmentOverviewSummary = { total: number; nonOperational: number; expiringDocuments: number };
...
export function EquipmentOverviewPage() {
  ...
  const summaryQuery = useQuery({ queryKey: ["equipment-overview-summary", filters], queryFn: async () => {
    const params = new URLSearchParams(); if (filters.search) params.set("search", filters.search); if (filters.faenaCodigo) params.set("faenaCodigo", filters.faenaCodigo); const normalizedZone = normalizeFaenaZone(filters.zona); if (normalizedZone) params.set("zona", normalizedZone); else if (filters.zona.trim()) params.set("zona", filters.zona.trim()); if (filters.tipoActivoCodigo) params.set("tipoActivoCodigo", filters.tipoActivoCodigo); if (filters.estadoOperacionalCodigo) params.set("estadoOperacionalCodigo", filters.estadoOperacionalCodigo); if (filters.tipoUbicacionFisica) params.set("tipoUbicacionFisica", filters.tipoUbicacionFisica); if (filters.tallerCodigo) params.set("tallerCodigo", filters.tallerCodigo); const result = await apiFetch<EquipmentOverviewSummary>("/api/assets/equipment-overview/summary?" + params.toString()); if (!result) throw new Error("No fue posible obtener el resumen de equipos."); return result;
  ...
  function applyFilters() { if (JSON.stringify(draft) === JSON.stringify(filters)) { void overviewQuery.refetch(); void summaryQuery.refetch(); } else setFilters({ ...draft }); }
  ...
  return <section className="equipment-theme-scope space-y-4">
    ...
    <EquipmentSummaryCards metrics={{ total: summaryQuery.data?.total ?? (summaryQuery.isLoading ? null : total), nonOperational: summaryQuery.data?.nonOperational ?? null, expiringDocuments: summaryQuery.data?.expiringDocuments ?? null }} unavailable={summaryQuery.isError} />
    ...
    <button className="secondary-button" type="button" onClick={() => { void overviewQuery.refetch(); void summaryQuery.refetch(); }}><RefreshCw className="h-4 w-4" />Actualizar resultados</button>
    <AssetEditorDialog open={newOpen} onClose={() => setNewOpen(false)} onSaved={assetCode => { setNewOpen(false); void summaryQuery.refetch(); navigate("/equipos/activos/" + encodeURIComponent(assetCode)); }} />
  </section>;
}