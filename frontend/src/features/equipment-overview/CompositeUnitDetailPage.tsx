...
const tabs = [["resumen", "Resumen"], ["composicion", "Composicion"], ["historial", "Historial de composicion"], ["documentos", "Documentos de componentes"]] as const;
...
function Composition({ unit, openAsset, onAction, canCompose }: { unit: Unit; openAsset: (code: string) => void; onAction: (mode: "mount" | "replace" | "unmount") => void; canCompose: boolean }) {
  ...
  return <><div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5"><div className="grid items-center gap-3 lg:grid-cols-[1fr_90px_1fr]"><ComponentCard label="Chasis" component={chasis} openAsset={openAsset} onAction={onAction} canCompose={canCompose} /><div className="text-center text-xs text-slate-500"><Truck className="mx-auto mb-2 h-7 w-7 text-sky-700" /><div className="h-0.5 bg-slate-400" /><b className="mt-2 block">Composicion vigente</b></div><ComponentCard label="Fabrica" component={fabrica} openAsset={openAsset} onAction={onAction} canCompose={canCompose} /></div></div><div className="mt-4 flex flex-wrap gap-2"><button className="secondary-button" type="button" disabled={!canCompose} onClick={() => onAction("mount")}>Montar componente</button><button className="secondary-button" disabled={!canCompose || !unit.composicion.vigentes.length} type="button" onClick={() => onAction("replace")}>Reemplazar componente</button></div></>;
}
...
function ComponentCard({ label, component, openAsset, onAction, canCompose }: { label: string; component?: UnitComponent; openAsset: (code: string) => void; onAction: (mode: "replace" | "unmount") => void; canCompose: boolean }) {
  return <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-[11px] font-extrabold uppercase tracking-[.12em] text-teal-700">Rol: {label}</p>{component ? <><h3 className="mt-1 font-semibold text-slate-900">{component.activoNombre || "NA"}</h3><p className="text-sm text-slate-500">{component.activoCodigo}</p><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><span className="rounded bg-slate-50 p-2">Estado<b className="mt-1 block">{na(component.estadoOperacionalCodigo)}</b></span><span className="rounded bg-slate-50 p-2">Medicion<b className="mt-1 block">NA</b></span></div><div className="mt-4 flex gap-2"><button className="secondary-button text-xs" type="button" onClick={() => openAsset(component.activoCodigo)}>Ver detalle</button><button className="secondary-button text-xs" type="button" disabled={!canCompose} onClick={() => onAction("replace")}>Reemplazar</button><button className="secondary-button text-xs text-red-700" disabled={!canCompose} type="button" onClick={() => onAction("unmount")}>Desmontar</button></div></> : <p className="mt-3 text-sm text-slate-500">NA</p>}</article>;
}
