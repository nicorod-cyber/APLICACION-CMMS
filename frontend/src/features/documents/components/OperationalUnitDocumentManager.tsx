...
export function OperationalUnitDocumentManager({ unitCode }: { unitCode: string }) {
  ...
  const mutation = useMutation({
    mutationFn: async ({ action, formValue, text }: { action: Exclude<ActiveAction, null>; formValue: DocumentFormValue; text: string }) => {
      ...
      if (kind === "upload") {
        ...
        return documentApi.uploadUnitRequirement(unitCode, row.requirementKey, { tipoDocumento: row.documentTypeCode, archivo: formValue.archivo, fechaEmision: formValue.fechaEmision || null, fechaVencimiento: formValue.fechaVencimiento || null, observaciones: formValue.observaciones || null });
      }
      ...
      if (kind === "replace") {
        ...
        return documentApi.replaceUnitDocument(unitCode, row.documentId, { tipoDocumento: row.documentTypeCode, archivo: formValue.archivo, fechaEmision: formValue.fechaEmision || null, fechaVencimiento: formValue.fechaVencimiento || null, observaciones: formValue.observaciones });
      }
      ...
      return documentApi.updateUnitDocument(unitCode, document.documentoId, { fechaEmision: formValue.fechaEmision || null, fechaVencimiento: formValue.fechaVencimiento || null, reason: formValue.observaciones });
    },
  ...
  const canSubmit = active?.kind === "upload" ? !!form.archivo : active?.kind === "replace" ? !!form.archivo && !!form.observaciones.trim() : active?.kind === "edit" ? !!form.observaciones.trim() : active?.kind === "reject" || active?.kind === "annul" ? !!reason.trim() : true;
