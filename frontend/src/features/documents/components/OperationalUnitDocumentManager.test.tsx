...
const view: OperationalUnitDocumentView = {
  ...
  rows: [
    { requirementKey: "chasis-key", documentTypeCode: "REVTEC", documentTypeName: "Revisión técnica", mandatory: true, critical: true, blocksAvailability: true, requiresExpirationDate: true, alertDays: 30, status: "PendienteCarga", canUpload: true, canReplace: false, canValidate: false, canReject: false, canAnnul: false, technicalOwnerRole: "CHASIS", technicalOwnerAssetCode: "CH-1", technicalOwnerAssetName: "Chasis", matrixId: "m1", matrixItemId: "i1" },
...
describe("OperationalUnitDocumentManager", () => {
  it("shows one consolidated table without component selectors and uploads from the selected requirement", async () => {
    ...
    renderUi();
    expect(await screen.findByText("Revisión técnica")).toBeInTheDocument();
