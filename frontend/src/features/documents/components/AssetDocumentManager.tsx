...
type RequirementSelection = { type: string; requiresExpirationDate: boolean; faenaCode?: string | null };
...
export function AssetDocumentManager({ assetCode, ownerLabel, matrix: suppliedMatrix }: Props) {
  ...
  const selectionFor = (row: DocumentMatrixRow): RequirementSelection => ({ type: row.tipoDocumento, requiresExpirationDate: !!row.requiereFechaVencimiento, faenaCode: row.faenaCodigo });
  ...
  return <section className="space-y-4">
    ...
    <DocumentEditorDialog open={createRequirement !== null} mode="create" entityCode={assetCode} documentType={createRequirement?.type} faenaCode={createRequirement?.faenaCode} requiresExpirationDate={createRequirement?.requiresExpirationDate} onClose={closeEditor} onSuccess={closeEditor} />
    <DocumentEditorDialog open={editing} mode="edit" document={selected} entityCode={assetCode} documentType={selected?.tipoDocumento} faenaCode={requirementForSelected?.faenaCodigo} requiresExpirationDate={requirementForSelected?.requiereFechaVencimiento} onClose={closeEditor} onSuccess={closeEditor} />
    <DocumentVersionsDialog open={!!versionsId} documentId={versionsId} onClose={() => setVersionsId(null)} />
    <DocumentReplacementDialog open={showReplacement} document={selected} faenaCode={requirementForSelected?.faenaCodigo} requiresExpirationDate={requirementForSelected?.requiereFechaVencimiento} onClose={closeReplacement} onSuccess={closeReplacement} />
