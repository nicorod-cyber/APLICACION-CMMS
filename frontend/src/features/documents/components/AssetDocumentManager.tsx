...
export function AssetDocumentManager({ assetCode, ownerLabel, matrix }: Props) {
  ...
  return <section className="space-y-4">
    ...
    {matrix ? matrix.length ? <div className="overflow-x-auto rounded-lg border border-slate-200"><table className="min-w-full text-sm"><thead><tr><th className="p-2 text-left">Documento</th><th className="p-2 text-left">Estado</th><th className="p-2 text-left">Obligatorio</th><th className="p-2 text-left">Vencimiento</th></tr></thead><tbody>{matrix.map(item => <tr className="border-t" key={item.tipoDocumento}><td className="p-2"><b className="block">{typeName(item.tipoDocumento)}</b><small className="text-slate-500">{item.tipoDocumento}</small></td><td className="p-2"><DocumentStatusBadge status={item.estado} />{item.motivoPendencia ? <small className="mt-1 block text-slate-500">{item.motivoPendencia}</small> : null}</td><td className="p-2">{item.obligatorio ? "Sí" : "No"}</td><td className="p-2">{date(item.fechaVencimiento)}{item.diasParaVencer !== null && item.diasParaVencer !== undefined ? <small className="mt-1 block text-slate-500">{item.diasParaVencer} días</small> : null}</td></tr>)}</tbody></table></div> : <p className="text-sm text-slate-500">Sin requisitos documentales aplicables.</p> : null}
