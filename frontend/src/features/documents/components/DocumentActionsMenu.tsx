...
export function DocumentActionsMenu({ document, capabilities, onEdit, onVersions, onReplace, onValidate, onReject, onAnnul }: Props) {
  ...
  return <div className="flex flex-wrap gap-1">
    {safeDocumentUrl(document.sharePointUrl) ? <DocumentLink aria-label="Abrir" className="secondary-button text-xs" href={safeDocumentUrl(document.sharePointUrl)} rel="noreferrer" target="_blank"><ExternalLink className="h-3 w-3" />Abrir</DocumentLink> : null}
    {safeDocumentUrl(document.sharePointUrl) ? <DocumentLink aria-label="Descargar" className="secondary-button text-xs" download href={safeDocumentUrl(document.sharePointUrl)}>Descargar</DocumentLink> : null}
