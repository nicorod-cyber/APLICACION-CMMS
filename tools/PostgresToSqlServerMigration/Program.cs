...
internal static class MigrationRunner
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        try
        {
            ...
            foreach (var table in tables)
            {
                ...
                var columns = sourceColumns.Intersect(destinationColumns, StringComparer.OrdinalIgnoreCase)
                ...
                if (!options.AllowNonEmptyTarget && !options.ValidateOnly && destinationCount != 0)
                    throw new InvalidOperationException($"El destino ya contiene {destinationCount} filas en dbo.{table}. Use una BD nueva; la herramienta no sobrescribe datos.");
            ...
            if (!options.DryRun && !options.ValidateOnly)
            {
                ...
                try
                {
                    foreach (var table in tableSpecs) await ExecuteSqlAsync(destination, transaction, $"ALTER TABLE dbo.{SqlName(table.Name)} NOCHECK CONSTRAINT ALL;");
    ...
    private static async Task<string[]> GetPostgreSqlColumnsAsync(NpgsqlConnection connection, string table) => await ReadStringsAsync(connection, "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = @table ORDER BY ordinal_position;", ("@table", table));
    private static async Task<string[]> GetSqlServerColumnsAsync(SqlConnection connection, string table) => await ReadStringsAsync(connection, "SELECT column_name FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = @table ORDER BY ordinal_position;", ("@table", table));
    ...
    {
        var names = string.Join(", ", table.Columns.Select(PostgreSqlName));
    ...
    private static async Task<string> ChecksumPostgreSqlAsync(NpgsqlConnection connection, TableSpec table) => await ChecksumAsync(connection, $"SELECT {string.Join(", ", table.Columns.Select(PostgreSqlName))} FROM public.{PostgreSqlName(table.Name)}{await PostgreSqlOrderByAsync(connection, table.Name)};");
    private static async Task<string> ChecksumSqlServerAsync(SqlConnection connection, TableSpec table) => await ChecksumAsync(connection, $"SELECT {string.Join(", ", table.Columns.Select(SqlName))} FROM dbo.{SqlName(table.Name)}{await SqlServerOrderByAsync(connection, table.Name)};");
    ...
    private sealed record TableSpec(string Name, string[] Columns, long SourceRows, long DestinationRows);
    ...
    private sealed record Options(string SourceConnectionString, string DestinationConnectionString, string ReportPath, bool DryRun, bool AllowNonEmptyTarget, bool ValidateOnly, string ExpectedMigration)
    {
        public static Options Parse(string[] args)
        {
            ...
            for (var index = 0; index < args.Length; index++)
            {
                ...
                if (argument is "--dry-run" or "--allow-nonempty-target" or "--validate-only") { values[argument] = "true"; continue; }
            ...
            return new Options(source, destination, values.GetValueOrDefault("--report") ?? $"postgres-to-sqlserver-report-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}.json", values.ContainsKey("--dry-run"), values.ContainsKey("--allow-nonempty-target"), values.ContainsKey("--validate-only"), values.GetValueOrDefault("--expected-migration") ?? BaselineMigration);
        }
        public static void PrintUsage() => Console.Error.WriteLine("Uso: dotnet run --project tools/PostgresToSqlServerMigration -- --source <postgres-cs> --destination <sqlserver-cs> [--report archivo.json] [--dry-run] [--validate-only] [--allow-nonempty-target]");
    }
}